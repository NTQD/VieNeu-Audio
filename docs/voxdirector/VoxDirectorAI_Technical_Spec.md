...
The team noticed the former Beta (terminology consistency) and Gamma (expression-word insertion) were doing the same _shape_ of work — look up a team-curated table, patch text accordingly, skip rather than guess when uncertain. They are merged into one agent. The former Delta (QA) is renumbered as the third agent in sequence.
...
| Gamma    | Expression-word insertion                 | _(retired — merged into Beta)_ | —                                                                                                         |
...
| ----------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `text_normalizer.py`    | Normalizes raw Vietnamese text into spoken-word form.                  | No change. Must run _after_ Beta and _after_ the sentinel marker has been stripped (Section 7.2) — normalizer should never see the marker token. |
...
3. **Optional manual inputs:** background image, background music, base pause duration (the _default_ inter-chunk silence — separate from Alpha's _extra-long_ dramatic pauses, see Section 7.2), subtitle burn-in toggle, QA (Gamma) toggle.
...
```json
{
  ...
  "emotion_flagged_segments": [
    {
      "quoted_text": "Cô ấy cười nói: \"Đúng vậy đó!\"",
      "emotion_label": "cuoi",
  ...
  "pause_points": [
    {
      "quoted_text": "...",
      "reason": "scene transition / long dramatic silence implied",
...
2. Expression pass: for each `emotion_flagged_segments` entry, locate `quoted_text` within _this same_ `chapter_text` (should be a reliable exact/near-exact match since Beta is working on the original chapter text directly, not a downstream-modified copy). If the lexicon's candidate word for that label isn't already present, insert one (varied across the chapter, not always the same word). If no match is found, skip and note why.
...
```json
{
  ...
  "pause_report": [{ "matched": true, "skipped_reason": null }]
}
```
...
### 8.1.1 Layout Structure — Two-Column Desktop, Stacked Mobile

**Desktop (primary target):** Two-column layout.

- **Left column — Input & Options:** text input area (paste/drag-drop) at top. Immediately below it, once Alpha finishes analysis: detected genre + voice suggestion (editable dropdown), shown inline in the same column — not a separate step or screen. Below that, an **"Advanced options"** collapsible block, closed by default, containing: background image upload, background music upload, pause-duration adjustment, subtitle burn-in toggle, QA toggle. A primary "Start Processing" button sits at the bottom of this column.
- **Right column — Work Area:** empty/placeholder initially (before processing starts). Once processing begins, this column displays the WebSocket-driven progress stages in place. Once complete, the same column shows the result: audio/video preview player, quality summary (if QA enabled), the segment list with per-segment playback and "re-render this segment," and the export buttons.

Both columns are visible simultaneously at all times on desktop — the user never loses sight of their input while watching progress or reviewing results.

**Mobile (secondary target — must work correctly, not required to be pixel-optimized):** Below a standard responsive breakpoint (e.g. `md` in Tailwind, ~768px), the two columns **stack vertically**: left column content first (input + options + start button), right column content (progress/result) appears below it. Functional flow is identical to desktop — no separate mobile-specific flow or reduced feature set. This is a standard responsive reflow, not a distinct mobile design.

**New-term confirmation panel** (Beta's terminology output) appears as a small dismissible overlay/panel when triggered, on top of whichever column layout is active — same behavior on desktop and mobile.
